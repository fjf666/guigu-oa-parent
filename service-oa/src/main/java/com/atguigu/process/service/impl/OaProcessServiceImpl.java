...
public class OaProcessServiceImpl extends ServiceImpl<OaProcessMapper, Process> implements OaProcessService {
    ...
    public IPage<ProcessVo> findFindPending(Page<Process> pageParam) {
        ...
        //4.封装返回IPage对象
        /*
        pageParam.getCurrent() 当前页
        pageParam.getSize()  每页显示数
        totalCount  总记录数
         */
    ...
    public void approve(ApprovalVo approvalVo) {
        ...
        //4.查询下一个审批人(上面审批后到下一个审批人)，更新流程表记录process表记录
        Process process = baseMapper.selectById(approvalVo.getProcessId());
        //查询任务
        List<Task> taskList = this.getCurrentTaskList(process.getProcessInstanceId());
        if (!CollectionUtils.isEmpty(taskList)){ //要是等于空就没有下一个审批人了
