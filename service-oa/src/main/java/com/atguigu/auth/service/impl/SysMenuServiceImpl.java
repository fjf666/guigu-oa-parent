...
//        }
    ...
    public List<SysMenu> finMenuByRoleId(Long roleId) {
        ...
        //3.1拿着菜单id和所有对应菜单集合里面id进行比较，如果相同封装
        for (SysMenu sysMenu : allSysMenuList) {
            if (menuIdList.contains(sysMenu.getId())){
                sysMenu.setSelect(true); //是否被选中 每次查看都全部变化，之前的true和false没影响
            }else{
        ...
        //4.返回规定格式菜单列表
        List<SysMenu> sysMenusList = MenuHelper.buildTree(allSysMenuList);
        return sysMenusList;
    ...
    public List<RouterVo> findUserMenuListByUserId(Long userId) {
        ...
        //构建成框架要求的路由结构
        List<RouterVo> routerVos = this.buildRouter(sysMenuTreeList);
        return routerVos;
    }
    ...
    private List<RouterVo> buildRouter(List<SysMenu> menus) {
        ...
        for (SysMenu menu : menus) {
            ...
            if (menu.getType().intValue() == 1){ //type 0目录，1菜单，2按钮
                //在1（菜单）加载出来下面2（按钮）隐藏路由
                List<SysMenu> hiddenMenuList = children.stream()
                        .filter(item -> !StringUtils.isEmpty(item.getComponent()))
                        .collect(Collectors.toList());
                for (SysMenu hiddenSysMenu : hiddenMenuList) {
                    RouterVo hiddenRouter = new RouterVo();
                    hiddenRouter.setHidden(true);
                    hiddenRouter.setAlwaysShow(false);
                    hiddenRouter.setPath(getRouterPath(hiddenSysMenu));
                    hiddenRouter.setComponent(hiddenSysMenu.getComponent());
                    hiddenRouter.setMeta(new MetaVo(hiddenSysMenu.getName(), hiddenSysMenu.getIcon()));
    ...
    public String getRouterPath(SysMenu menu) {
        ...
        if(menu.getParentId().intValue() != 0) { //不是等于0 时没有 /
